...
def upload_to_mega(zip_name):
    ...
    try:
        ...
        subprocess.run(['mega-mkdir', '/proton-firefox'], capture_output=True, text=True)
        
        # Upload the zip file to /part2
        result = subprocess.run(['mega-put', zip_name, '/proton-firefox'], check=True, capture_output=True, text=True)
